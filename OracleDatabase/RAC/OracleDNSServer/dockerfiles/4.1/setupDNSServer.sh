...
setEnvVariables()
{
  ...
  if [ -z ${CORP_DNS_DOMAIN_1} ]; then
     CORP_DNS_DOMAIN_1="us.oracle.com"
  fi

  if [ -z ${CORP_DNS_DOMAIN_2} ]; then
     CORP_DNS_DOMAIN_2="dbdevtestphx.oraclevcn.com"
  fi

  if [ -z ${CORP_DNS_SERVERS} ]; then
  error_exit "CORP DNS Servers cannot be set empty, please read /etc/resolv.conf and pass nameserver as comma separated strings"
  else
  FORWARDERS_TEMP=$( echo $CORP_DNS_SERVERS | sed 's/,/;@@ /g' )
  export FORWARDERS_TEMP=$FORWARDERS_TEMP
  CORP_DNS_SERVERS_TEMP=$( echo $CORP_DNS_SERVERS | sed 's/,/; /g')
  export CORP_DNS_SERVERS_TEMP=$CORP_DNS_SERVERS_TEMP
  print_message "CORP_DNS_SERVERS_TEMP is $CORP_DNS_SERVERS_TEMP"
  CORP_DNS_SERVERS=$( echo "$CORP_DNS_SERVERS_TEMP;" ) 
  print_message "CORP_DNS_SERVERS is $CORP_DNS_SERVERS" 
  export CORP_DNS_SERVERS=$CORP_DNS_SERVERS
  FORWARDERS=$( echo "$FORWARDERS_TEMP;" | tr '@@' '\n' )
  export FORWARDERS=$FORWARDERS
  print_message "FORWARDERS are $FORWARDERS"
  fi
...
setupNamed()
{
   ...
   sed -i -e "s|###CORP_DNS_DOMAIN_1###|$CORP_DNS_DOMAIN_1|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_SAMPLE_FILE
   sed -i -e "s|###CORP_DNS_DOMAIN_2###|$CORP_DNS_DOMAIN_2|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_SAMPLE_FILE
   sed -i -e "s|###CORP_DNS_SERVERS###|$CORP_DNS_SERVERS|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_SAMPLE_FILE
   ...
   sed -i -e "s|###RAC_PUBLIC_SUBNET###|$RAC_PUBLIC_SUBNET|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_SAMPLE_FILE
   sed -i -e "s|###NAMED_SAMPLE_FILE###|$NAMED_SAMPLE_FILE|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_CONF_FILE
   sed -i -e "s|###CORP_DNS_SERVERS###|$CORP_DNS_SERVERS|g" ${NAMED_CHROOT_ETC_DIR}/$NAMED_CONF_FILE
}
